...
namespace t1m::internal
{
  ...
  public:
    ...
    ScatterVector(MArray::len_vector lengths, MArray::len_vector strides, const std::vector<size_t> &indices)
    {
  ...
  std::vector<std::vector<size_t>> many_to_many(const std::vector<size_t> &values1, const std::vector<size_t> &values2)
  {
    std::vector<std::vector<size_t>> res;
    res.reserve(values1.size());

    for (const auto& summand : values1)
  ...
  std::vector<std::vector<size_t>> multi_level(std::vector<std::vector<size_t>> levels)
  {
    ...
    for (const auto& comb_it : combined)
    {
