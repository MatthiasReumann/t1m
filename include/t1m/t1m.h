...
void contract(const T alpha, const tensor<T, ndim_a>& a, std::string labels_a,
  ...
  scatter::block_layout<T, ndim_a> matr_a(a, indices.AI, indices.AP,
                                          blocksizes.MR, blocksizes.KP);
  scatter::block_layout<T, ndim_b> matr_b(b, indices.BP, indices.BJ,
                                          blocksizes.KP, blocksizes.NR);
  scatter::block_layout<T, ndim_c> matr_c(c, indices.CI, indices.CJ,
                                          blocksizes.MR, blocksizes.NR);

  packing::lhs<T> wrksp_a(blocksizes.MC * blocksizes.KC);
  packing::rhs<T> wrksp_b(blocksizes.KC * blocksizes.NC);
