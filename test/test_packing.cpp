...
TEST_CASE("packing of A tensor") {
  SUBCASE("quadratic block") {
    ...
    scatter::matrix_view matrix{layout.rs,  layout.cs, layout.br,
    ...
    std::array<float, workspace_size> expected{1, 2, 5, 6, 9,  10, 13, 14,
                                               3, 4, 7, 8, 11, 12, 15, 16};
    workspace.pack(matrix, elems.data(), M, K);
  ...
  //     // 3  7  11  15
  //     // 4  8  12  16
  //     scatter::matrix_view<float, 3> layout(t, {0, 1}, m, {2}, k);
  ...
  //     workspace.pack(layout, elems.data(), M, K);
